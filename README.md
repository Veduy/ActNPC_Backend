...
`actions`는 NPC가 순서대로 실행할 액션 목록입니다. `object_name`은 허용 오브젝트 목록에 있는 이름만 사용할 수 있고, `object_id`와 `position`은 Unity 상태 조회 결과로 실제 씬 인스턴스가 선택된 경우에 채워집니다.
...
백엔드는 WebSocket 연결마다 독립된 세션 메모리를 생성합니다. 이 메모리는 대화 담당 LLM과 명령 담당 플래너가 같은 사용자 흐름을 이어서 이해하기 위한 컨텍스트로 사용됩니다.

메모리에 저장하는 정보는 다음과 같습니다.

- 최근 사용자/assistant 메시지
- 오래된 대화 내용을 압축한 요약
- 마지막으로 생성된 명령 결과

대화 입력으로 분류된 경우에는 요약과 최근 메시지를 대화 LLM에 전달합니다. 이를 통해 NPC가 이전 대화를 참고해 자연스럽게 응답할 수 있습니다.

명령 입력으로 분류된 경우에는 요약, 최근 메시지, 마지막 명령 결과를 플래너에 전달합니다. 이를 통해 "그거 내려놔", "아까 말한 거 가져와"처럼 이전 맥락을 참조하는 명령을 해석할 수 있게 했습니다.

최근 메시지는 일정 개수 이상 쌓이면 오래된 내용을 별도 LLM 호출로 요약해 보관합니다. 이 방식은 전체 대화 로그를 계속 프롬프트에 넣지 않으면서도, 사용자 선호나 이전 지시, 해결되지 않은 참조 같은 정보를 유지하기 위한 구조입니다. 세션 메모리는 현재 WebSocket 연결 단위로 유지되며, 연결이 종료되면 삭제됩니다.
...
- 세션별 대화 요약, 최근 메시지, 마지막 명령 컨텍스트 관리
