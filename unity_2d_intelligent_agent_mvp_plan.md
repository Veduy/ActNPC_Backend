...
현재 구현된 `/command` 요청에는 다음 필드만 포함한다.
- `message`

`action`, `destination`, `item`, `message` 응답한다. 현재 씬의 오브젝트 목록, 아이템 좌표, 아이템 상태, NPC 상태 등은 향후 Planner/RAG 단계에서 필요할 때 MCP Tool을 통해 조회한다.
...
Knowledge Store는 게임 세계에 대한 배경지식과 규칙을 저장한다.
...
백엔드는 다음 형태로 응답한다.
*추후 수정* "item"은 string이 아닌 item id를 넣을것.-> Db
